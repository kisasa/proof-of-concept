 ...
 * rather than calling console.* directly — this package's own exception, same
 * as webhook-listener/src/logger.ts and specialist-runner/src/logger.ts are
 * for theirs.
 *
 * Not used inside activity or workflow code that runs under Temporal's own
 * sandbox/replay model — those use `@temporalio/activity`'s Context.log /
 * `@temporalio/workflow`'s log, which route through Temporal's own logging
 * sink instead. This logger is for worker.ts's own startup/shutdown lines,
 * outside any workflow or activity execution.
 *
