...
- Then set the claim label:
  - `claim:proven` if every step worked;
  - `claim:failed` otherwise, with the failed steps named.
- No reply is not a pass. The label stays `claim:awaiting-demo` until the
  human answers.
...
| Label | On | Meaning |
|---|---|---|
| `claim:awaiting-demo` | epic | Ready to be watched |
| `claim:proven` | epic | Every step watched working |
| `claim:failed` | epic | At least one step did not work |

These are proposals and unsettled. No plumbing reads them.

## Open

- **No lane runs this skill yet.** The listener has no demo lane, and nothing
  applies `claim:awaiting-demo` when the last story merges. Wiring either is a
  plumbing change.
